...
class BaseTransform(nn.Module):
    ...
    def get_geometry(
        ...
        **kwargs,
        ...
        # Use more stable inverse computation to avoid CUDA magma errors
        try:
            post_rots_inv = torch.linalg.pinv(post_rots)
        except:
        ...
        # Use more stable inverse computation to avoid CUDA magma errors
        try:
            intrins_inv = torch.linalg.pinv(intrins)
        except:
            intrins_inv = torch.inverse(intrins)
        combine = camera2lidar_rots.matmul(intrins_inv)
    ...
    def forward(
        ...
        **kwargs,
        ...
        for b in range(batch_size):
            ...
            # Use more stable inverse computation to avoid CUDA magma errors
            try:
                lidar_aug_inv = torch.linalg.pinv(cur_lidar_aug_matrix[:3, :3])
            except:
                lidar_aug_inv = torch.inverse(cur_lidar_aug_matrix[:3, :3])
            cur_coords = lidar_aug_inv.matmul(
