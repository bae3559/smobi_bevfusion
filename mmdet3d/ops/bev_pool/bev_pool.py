...
class QuickCumsumCuda(torch.autograd.Function):
    ...
    def forward(ctx, x, geom_feats, ranks, B, D, H, W):
        ...
        # Debug information for empty interval_starts
        if len(interval_starts) == 0:
            print(f"DEBUG: Empty interval_starts detected!")
            print(f"x.shape: {x.shape}")
            print(f"geom_feats.shape: {geom_feats.shape}")
            print(f"ranks.shape: {ranks.shape}")
            print(f"ranks unique values: {torch.unique(ranks).cpu().numpy()}")
            print(f"B={B}, D={D}, H={H}, W={W}")
            print(f"kept sum: {kept.sum()}")
            print(f"All ranks equal? {len(torch.unique(ranks)) == 1}")
            # Return zeros instead of raising error to continue debugging
            return torch.zeros((B, H, W, D, x.shape[-1]), device=x.device, dtype=x.dtype)
